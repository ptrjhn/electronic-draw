...
new Vue({
  ...
  methods: {
    getMembers: function(page) {
      axios.get('/api/administration/members?page='+page + '&search='+this.searchText).then((response) => {
        this.members = response.data.data;
        this.pagination = response.data;
      });
    },
    searchMember() {
      axios.get('/api/administration/members?page=1&search='+this.searchText).then((response) => {
         this.members = response.data.data;
        this.pagination = response.data;
    ...
    createMember: function() {
      ...
      axios.post('/api/administration/members',input).then((response) => {
    ...
    deleteMember: function(member) {
      axios.delete('/api/administration/members/'+member.id).then((response) => {
        this.changePage(this.pagination.current_page);
    ...
    updateMember: function(id) {
      var input = this.formData;
      axios.put('/api/administration/members/'+id,input).then((response) => {
        this.getMembers(1);
        Modal.hide("#member-form-modal");
